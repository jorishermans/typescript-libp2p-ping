declare module 'libp2p' {
  ...
  class LibP2p {
    ...
    ping(peerInfo: PeerInfo, cb: (err: any, ev: any) => any): void;
    start(): Promise<void>;
    stop(): Promise<void>;
  }
...
declare module 'libp2p/src/ping' {
  import PeerInfo from 'peer-info'

  export type Events = 'ping' | 'error';

  class Ping extends Event {
    constructor(swarm: any, peer: PeerInfo)
    on(event: Events, cb: (event: any) => any): this
    once(event: Events, cb: (event: any) => any): this
    removeListener(event: Events, cb: (event: any) => any): this
